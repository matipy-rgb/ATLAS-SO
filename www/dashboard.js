(function () {
    ...
    function getAttention(data) {
        ...
        (window.ATLAS_IS_HR_ADMIN ? data.hrAbsences : []).filter(item => !item.actualReturnDate && !item.cancelled).forEach(item => {
    ...
    function renderModules(data) {
        ...
        moduleOverview.innerHTML = cards.filter(card => card.key !== "rrhh" || window.ATLAS_IS_HR_ADMIN).map(card => `
